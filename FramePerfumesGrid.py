...
class FramePerfumes(guiperfumes.FramePerfumesGrid):
	...
	def salvarPerfume( self, event ):
		...
		status,message=AcessandoBD.salvarPerfumes(listaPerfumes=perfumes) #Chama a função salvarPerfumes, passando a lista de perfumes
		if status:
	...
	def exibirTabela(self):
		perfumes=AcessandoBD.listarPerfumes() #Recupera a lista de perfumes
	...
	def criarLinha(self,id_perfume=None,nome_perfume=None,quantidade=None,marca=None,fixacao=None,volume=None):
		...
		self.gridPerfumes.SetCellEditor(linha,2,wx.wx.Grid.GridCellNumberEditor(min=1,max=1000)) #Define que a coluna quantidade só pode receber inteiros entre 1 e 1000
		'''
		...
		'''
		self.gridPerfumes.SetCellEditor(linha,3,wx.wx.grid.GridCellChoiceEditor(choices=AcessandoBD.listarVolumeNome()))
		self.gridPerfumes.SetCellEditor(linha, 4,
										wx.wx.grid.GridCellChoiceEditor(choices=AcessandoBD.listarMarcaNome()))
		self.gridPerfumes.SetCellEditor(linha, 5,
										wx.wx.grid.GridCellChoiceEditor(choices=AcessandoBD.listarFixacaoNome()))
